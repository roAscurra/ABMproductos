...
interface ModalDeleteProductsProps {
    show: boolean;
    onHide: () => void;
    product: IArticuloManufacturado | null;
    onDelete: () => void;
}

const ModalDelete: React.FC<ModalDeleteProductsProps> = ({ show, onHide, product }) => {
    ...
    const handleDelete = async () => {
        try {
            if (product && product.id) {
                await productService.delete(url + 'api/producto', product.id.toString());
